...
class Connection:
    ...
    def todosUsers(self):
        # TODOS OS USUÁRIOS
        self.json_responseUser = requests.get(self.url + 'users/')
    ...
    def tipo_colaborador(self, dadosJson):
        ...
        if (dadosJson.status_code == 200):
            ...
            if (dadosJson['type']['type'] == 'AHK'):
                return ['ahk', self.idCardColadorador, self.edvColadorador, self.nameColadorador, self.dataNascFormat2]
                print('user AHK')
            ...
            if (dadosJson['type']['type'] == 'Gestor'):
                ...
                print('user ADM - Gestor')

            if (dadosJson['type']['type'] == 'Instrutor'):
                return ['adm', self.idCardColadorador, self.edvColadorador, self.nameColadorador, self.dataNascFormat2]
                print('user adm - Instrutor')
    ...
    def adicionar_cadastro(self, tag_cartao, nome, classe, edv, datanasc):
        ...
        if self.tipo_classe(classe) ==1:
            ...
            try:
                urlPost = self.url + 'users/'

                post = requests.post(urlPost, json= body)

                if post.status_code==200:
                    print('post realizado', post)
                    try:
                        urlGet = requests.get(self.url + 'usersid/' + tag_cartao)

                        if urlGet.status_code==200:
                            urlGet = urlGet.json()
                            urlID = urlGet['id']

                        urlApprentice = self.url + 'ap/' + tag_cartao

                        gett = requests.get(urlApprentice)

                        body = [
                            {
                                "course": 2,
                                "idApprenticeFK": int(urlID)
                            }
                        ]

                        post = requests.post(urlPostApprentice, json=body)
        ...
        #
        # adicionar = ("INSERT INTO Usuarios (ID_Cartao, Nome, EDV, Classe, Data_Nascimento)  VALUES ('" + tag_cartao + "','" + nome + "','" + edv + "'," + classe + "," + datanasc + ")")  # inserir Dados
...
class Sair():
    ...
    def sairBanco(self):
        ...
        try:
            ...
            url = 'http://localhost:8000/releasemachines/1/'
